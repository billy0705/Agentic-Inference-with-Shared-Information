...
while IFS=$'\t' read -r benchmark methods limit output_dir resume_run max_steps think_mode single_agent_min_steps single_agent_max_steps debate_rounds max_tokens; do
  evaluation_args=(
    --benchmark "$benchmark"
    --methods "$methods"
    --limit "$limit"
